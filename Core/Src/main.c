...
uint8_t Bt_Data[1]={1};
uint8_t Mode=1,Speed=0,Shearing=0,Joystick=0,Steering_Mode=0, Pot_Angle=0, Joystick_Temp=0, Shearing_Temp=0;
...
/*																								ARM DEFINITIONS																							*/		
float L_Arm_Speed=0, R_Arm_Speed=0, L_Arm_Speed_Temp=0, R_Arm_Speed_Temp=0, Pitch_Arm_Speed=0, Pitch_Arm_Speed_Temp=0, L_Arms_Speed=0, L_Arms_Speed_Temp=0, Tri_Arm_Speed=0;
...
uint16_t LA_Home_Pos = 350, RA_Home_Pos= 481, PA_Home_Pos = 4;
/*																								ARM DEFINITIONS																							*/	
...
int Flaps_Target = 54, Sensed_Count=0 ;        //40-20
	float Flap_Kp = 3, Pitch_Kp=1 ; //3,2
int  Flap_Error=0;
	
	uint16_t FL_Home_Pos = 77 , FR_Home_Pos = 0, RL_Home_Pos = 0, RR_Home_Pos = 0;
float FL_Raw =0, FR_Raw = 0, RL_Raw = 0, RR_Raw = 0;

int Width_Motor_Count=0, Left_Macro_Motor_Count=0, Width_Motor_Value=0, Left_Macro_Motor_Value=0, Right_Macro_Motor_Value=0, Right_Macro_Motor_Count=0; // Total Counts
...
int16_t LA_value, RA_value, PA_value;
uint8_t Prev_Joystick;

float Front_Arms_Speed = 0, Front_Arms_Speed_Temp = 0;
float P_Arm_Speed = 0, P_Arm_Speed_Temp = 0;
...
//	
//	}
	memcpy(&Absolute_Position[Node_Id],RxData, sizeof(float)); 
	
	LA_value = Absolute_Position[5];
	RA_value = Absolute_Position[6];
	PA_value = Absolute_Position[7];
	
}
float New_Sensor_Pos(double sensorvalue, double zero_pos)
{
double output;
...
//		  case 0x281: Node_Id[20]++; break;
		//Motor_Velocity[Received_Node_Id]	= CAN_Reception(MSB);    -->  ENEST_ID
			default: 																																													  break;
...
/* 1. Turn On buzzer if the Bt is not connected 
	...
	if (Steering_Mode == 2 && Mode != 2)
	{
		if (Prev_Joystick == 1 && Joystick == 0)
		{
			Joystick = 1;
		}
		else {}
...
//				Set_Motor_Velocity(19, 10);
//			}
//			HAL_Delay(1000);
//			for(uint16_t i = 0; i < 1000; i++){
//				Set_Motor_Velocity( 19 , -40);
//			}
//			HAL_Delay(4000);
			for (uint8_t i=0; i < 5 ; i++)
			{
				Set_Motor_Velocity( 18 , 35);  // Main paddle   //50
	//			HAL_Delay(100);
				//Set_Motor_Velocity( 17 , 30);// Selective  45
	//			HAL_Delay(100);
				Set_Motor_Velocity( 19 , 45); // Side Paddle   35
	//			HAL_Delay(100);
				Set_Motor_Velocity( 20 , 50 );  // Cutter   30
	//			HAL_Delay(100);
...
//			}
			...
			switch ( Joystick )
			{
				...
				case 3 : Width_Speed =  70;   break;
				case 4 : Width_Speed = -70;   break;
				default: break;
			}
...
/*					          		Course Correction														*/		
	...
	else if(Mode == 2 && Steering_Mode ==3)
	{
		if(Joystick != Joystick_Temp)
		{
			switch(Joystick)
			{
				case 0: Front_Arms_Speed = 0, P_Arm_Speed = 0; break;
				case 1:	Front_Arms_Speed = -5; break;
				case 2:	Front_Arms_Speed = 5; break;
				case 3:	P_Arm_Speed = -5; break;
				case 4:	P_Arm_Speed = 5; break;
				default: break;
			}
			Joystick_Temp = Joystick;
		}
		
		if(Front_Arms_Speed != Front_Arms_Speed_Temp)
		{
			Set_Motor_Velocity(5, Front_Arms_Speed);
			Set_Motor_Velocity(6, Front_Arms_Speed);
			Front_Arms_Speed_Temp = Front_Arms_Speed;
		}
		
		if(P_Arm_Speed != P_Arm_Speed_Temp)
		{
			Set_Motor_Velocity(7, P_Arm_Speed);
			P_Arm_Speed_Temp = P_Arm_Speed;
		}
	}
...
//	else if (  !Idle_Wheels ) { Vel_Limit = 20 + Speed * 20;}
//	else {}
	Vel_Limit = Speed * 16;                        //Vel_Limit = 10 + Speed * 20;

	if ( Mode != 2 || Joystick != 3 || Joystick != 4)                           //only Mode != 2
	{
		Turn_Ratio = Pot_Angle - 90;
...
void Front_Flap_Sensing()
{
	Front_Left_Bush = FL_Angle > 10 ? 1 : 0;               //Angle setpoint
	
	if ( Front_Left_Bush  ) Front_Bushes_Sensed = SET;       //if bush sensed
	else Front_Bushes_Sensed = NULL;                         //if bush not sensed
	...
	if(Mode != 3)
	{
		...
		First_Sense = 0; Sensed_Count =0;           //setting first sende to 0 as mode is not equal to 3
	}
	
	else if(Mode == 3)
	{
		if ( Front_Bushes_Sensed ) First_Sense = 1 ;     // setting first sense to 1 when bushes sensed when sensing mode is ON
		...
		if (First_Sense)             //if bushes sensed
		{
			Sensed_Count++;
			
			L_Arm_Speed = Front_Left_Bush == 1 ? ( Flaps_Target - FL_Angle ) * Flap_Kp : 0 ;   //Error Correction
			L_Arm_Speed = ( L_Arm_Speed > -5 && L_Arm_Speed < 5 ) ? 0 : L_Arm_Speed ;         
			
			R_Arm_Speed = L_Arm_Speed;           // Same speed for right arm as left arm
			Pitch_Arm_Speed = L_Arms_Speed / 2;   // half of the front arm speed to pitch arm
				
		}
		
		if ( First_Sense && !Front_Left_Bush )  // Bushes Sensed at First and Gap Detected while Running
		{
...
void EEPROM_Store_Data (void)
{
	...
	Left_Arm_Motor_Count  = Left_Arm_Motor_Value + LA_value;
	Right_Arm_Motor_Count = Right_Arm_Motor_Value + RA_value;
	Pitch_Arm_Motor_Count = Pitch_Arm_Motor_Value + PA_value;	
	//Left_Arm_Motor_Count = Left_Arm_Motor_Count == -1 ? 0 : Left_Arm_Motor_Count;
	memcpy(&Write_Value[12], &Left_Arm_Motor_Count, sizeof(Left_Arm_Motor_Count));
	memcpy(&Write_Value[4], &Right_Arm_Motor_Count, sizeof(Right_Arm_Motor_Count));
	memcpy(&Write_Value[8], &Pitch_Arm_Motor_Count, sizeof(Pitch_Arm_Motor_Count));
	
	
//	Width_Motor_Count = Width_Motor_Value + Absolute_Position_Int[8];	
//	Left_Macro_Motor_Count = Left_Macro_Motor_Value + Absolute_Position_Int[9];
//	Right_Macro_Motor_Count = Right_Macro_Motor_Value + Absolute_Position_Int[10];
//	memcpy(&Write_Value[12], &Width_Motor_Count, sizeof(Width_Motor_Count));
//	memcpy(&Write_Value[16], &Left_Macro_Motor_Count, sizeof(Left_Macro_Motor_Count));
//	memcpy(&Write_Value[20], &Right_Macro_Motor_Count, sizeof(Right_Macro_Motor_Count));
...
void Read_EEPROM_Data(void)
{		
	EEPROM_Read(3, 0, (uint8_t *)Read_Value, sizeof(Read_Value));
	memcpy(&Left_Arm_Motor_Value, &Read_Value[12],4 );	 				
	memcpy(&Right_Arm_Motor_Value, &Read_Value[4],4 );
	memcpy(&Pitch_Arm_Motor_Value, &Read_Value[8],4 );
//	memcpy(&Width_Motor_Value, &Read_Value[12],4 );
//	memcpy(&Left_Macro_Motor_Value, &Read_Value[16],4 );
//	memcpy(&Right_Macro_Motor_Value, &Read_Value[20],4 );
...
//	Width_Motor_Value 		 			 = Width_Motor_Value 						== -1 ? 0 : Width_Motor_Value;
//	Left_Macro_Motor_Value 		   = Left_Macro_Motor_Value 			== -1 ? 0 : Left_Macro_Motor_Value;
//	Right_Macro_Motor_Value      = Right_Macro_Motor_Value  		== -1 ? 0 : Right_Macro_Motor_Value;
}

void Z_Top_Flap_Sensing(void)
{

	Front_Left_Bush  =  FL_Angle > 30 ? 1 : 0;
	
	if ( Mode != 3 && Mode != 2)
	{
		L_Arm_Speed 		 = Left_Arm_Motor_Count <= 1 && Left_Arm_Motor_Count  >=-1 ? 0 : Left_Arm_Motor_Count < -1 ? 10 : -40;
		R_Arm_Speed 		 = Right_Arm_Motor_Count<= 1 && Right_Arm_Motor_Count >= -1 ? 0 : Right_Arm_Motor_Count < -1 ? 10 : -40;
	...
	else if (Mode == 3)
	{
		...
		L_Arm_Speed = L_Arm_Speed < 0 ? L_Arm_Speed - 15 : L_Arm_Speed;
		R_Arm_Speed = R_Arm_Speed < 0 ? R_Arm_Speed - 15 : R_Arm_Speed;
	}
	else
 {
	 //L_Arm_Speed = R_Arm_Speed = Pitch_Arm_Speed = 0;
	...
	//L_Arm_Speed = L_Arm_Speed > 0 && Left_Arm_Motor_Count >= 60 ? 10 : L_Arm_Speed < 0 && Left_Arm_Motor_Count <= 10 ? -10 : L_Arm_Speed ;
		L_Arm_Speed = L_Arm_Speed > 0 && Left_Arm_Motor_Count >= 70 ? 0 : L_Arm_Speed < 0 && Left_Arm_Motor_Count <= 0 ? 0 : L_Arm_Speed ;
	if( L_Arm_Speed_Temp != L_Arm_Speed )
	{
		R_Arm_Speed = R_Arm_Speed > 0 && Right_Arm_Motor_Count >= 60 ? 10 : R_Arm_Speed < 0 && Right_Arm_Motor_Count <= 10 ? -10 : R_Arm_Speed ;
		Set_Motor_Velocity (L_Arm , L_Arm_Speed );	
		L_Arm_Speed_Temp = L_Arm_Speed ;
	}
	
	R_Arm_Speed = R_Arm_Speed > 0 && Right_Arm_Motor_Count >= 70 ? 0 : R_Arm_Speed < 0 && Right_Arm_Motor_Count <= 0 ? 0 : R_Arm_Speed ;
	if( R_Arm_Speed_Temp != R_Arm_Speed )
	{
		Set_Motor_Velocity (R_Arm , R_Arm_Speed );	
		R_Arm_Speed_Temp = R_Arm_Speed ;
	...
	Pitch_Arm_Speed = Pitch_Arm_Speed > 0 && Pitch_Arm_Motor_Count >= 30  ? 5 : Pitch_Arm_Speed < 0 && Pitch_Arm_Motor_Count <= 10 ?-5 : Pitch_Arm_Speed ;
	Pitch_Arm_Speed = Pitch_Arm_Speed > 0 && Pitch_Arm_Motor_Count >= 40  ? 0 : Pitch_Arm_Speed < 0 && Pitch_Arm_Motor_Count <= 0 ? 0 : Pitch_Arm_Speed ;
	if( Pitch_Arm_Speed_Temp != Pitch_Arm_Speed ) 
	{
		Set_Motor_Velocity (P_Arm , Pitch_Arm_Speed );	
		Pitch_Arm_Speed_Temp = Pitch_Arm_Speed ;
